...
class AutocompleteService extends jsw.TypedProxy {
  ...
  void getPlacePredictions(AutocompletionRequest request, void callback(List<AutocompletePrediction> results, PlacesServiceStatus status)) {
    $unsafe.getPlacePredictions(request, (js.Proxy results, String status) => callback(jsw.JsArrayToListAdapter.castListOfSerializables(results, AutocompletePrediction.cast), PlacesServiceStatus.find(status)));
  }
  void getQueryPredictions(QueryAutocompletionRequest request, void callback(List<QueryAutocompletePrediction> results, PlacesServiceStatus status)) {
    $unsafe.getQueryPredictions(request, (js.Proxy results, String status) => callback(jsw.JsArrayToListAdapter.castListOfSerializables(results, QueryAutocompletionRequest.cast), PlacesServiceStatus.find(status)));
  }
}